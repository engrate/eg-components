...
// forked from design-sync lib/preview-gen-storybook.mjs — mirror Storybook's core
// actions argsEnhancer so argTypes[k].action injects a handler (Banner gates its
// dismiss button on `onDismiss &&`, so without this the preview loses real UI).
...
const COMPOSE = `function compose(S: any, key: string) {
  ...
  // Storybook ships addActionsFromArgTypes in core: any argType declaring
  // \`action\` receives a spy handler on every story, even when the story's own
  // args never mention it. Components that gate UI on the prop being present
  // (\`{onDismiss && <button/>}\`) therefore render MORE in storybook than a
  // preview that leaves it undefined. Mirror the enhancer with a no-op.
  for (const k of Object.keys(at)) {
    if (at[k] && at[k].action && args[k] === undefined) args[k] = () => {};
  }
