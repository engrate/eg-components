...
>(({ className, children, variant = 'default', ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'text-body-sm text-primary flex w-full items-center justify-between rounded-md border bg-white px-3 py-2 font-sans transition-colors',
      'data-placeholder:text-quaternary',
      'focus-visible:ring-1 focus-visible:outline-none',
      'disabled:cursor-not-allowed disabled:opacity-50',
      '[&>span]:line-clamp-1',
      variant === 'default'
        ? 'border-border focus-visible:ring-sunflower'
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'border-border text-primary relative z-50 max-h-96 min-w-32 overflow-hidden rounded-md border bg-white font-sans shadow-md',
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      'text-body-sm relative flex w-full cursor-default items-center rounded-sm py-2 pr-2 pl-8 outline-none select-none',
      'focus:bg-alt focus:text-primary',
